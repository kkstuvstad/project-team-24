...
public class Board {
	...
	//Check if the square occupied by this ship are already occupied.
	Square testSquare = null;
	//Check if the placement is valid on the grid
	if(isVertical) {
        ...
        if (x + ship.getLength() - 1 > width) {
			...
			// Before that it would check if any occupied square of this ship already occupied.
            for (int i = 0; i < ship.getLength(); i++) {
            	testSquare = new Square(x+i,y);
            	for(int j = 0;j < boardShips.size();j++){
            		for(int k = 0;k<boardShips.get(j).getLength();k++){
            			if(testSquare.getRow() == boardShips.get(j).getOccupiedSquares().get(k).getRow() && testSquare.getColumn() == boardShips.get(j).getOccupiedSquares().get(k).getColumn()){
            				return false;
						}
					}
				}
		...
		if ((int)(y-'A') + ship.getLength() >= length){
			return false;
		}
		else{
			//For the same purpose the square occupied would be checked

			for(int i = 0;i < ship.getLength(); i++){
				testSquare = new Square(x,(char)(y + i));
				//Loop the ships
				for(int j = 0;j < boardShips.size();j++){
					//Loop the squares
					for(int k = 0;k<boardShips.get(j).getLength();k++){
